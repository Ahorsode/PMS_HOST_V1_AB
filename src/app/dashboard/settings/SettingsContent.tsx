...
export function SettingsContent({ farm, inventory = [] }: SettingsContentProps) {
  ...
  const [licenseStatus, setLicenseStatus] = useState<string>(farm?.master_license_status || 'UNPAID');
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-5">
      ...
      <div className="md:col-span-3 space-y-5">
        ...
        {activeTab === 'farm' && (
          <>
            {/* Desktop Offline Bundle Callout Banner */}
            {licenseStatus !== 'PAID_AND_ACTIVE' ? (
              <Card className="border border-emerald-500/30 bg-gradient-to-br from-[#0c1914] via-[#09110d] to-[#040806] shadow-[0_8px_32px_rgba(0,0,0,0.5),0_0_20px_rgba(16,185,129,0.05)] overflow-hidden relative group">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1.5">
                      ...
                      <h3 className="text-xl font-bold text-white tracking-tight">
                        Deploy Poultry PMS Offline on Your Farm
                      </h3>
                      <p className="text-xs text-white/70 max-w-xl leading-relaxed">
                        Collect feeding stats, egg production, and mortality logs 100% offline with our native Windows app. Automatically syncs back to the cloud when internet restores.
                      </p>
                    </div>
                    <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 shrink-0">
                      <Button
                        onClick={() => setShowCheckoutModal(true)}
                        className="bg-emerald-500 hover:bg-emerald-400 text-black font-bold text-xs uppercase tracking-widest px-4 py-2.5 rounded-md shadow-lg shadow-emerald-500/10 flex items-center justify-center gap-1.5"
                      >
                      ...
                      <Button
                        ...
                        className="border-white/10 hover:bg-white/5 text-white/90 text-xs font-bold uppercase tracking-widest px-4 py-2.5 rounded-md"
                      >
                        Learn More
                      </Button>
                    </div>
              ...
              <Card className="border border-white/5 bg-zinc-950/40 overflow-hidden relative group">
                <CardContent className="p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-wider">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                        Desktop License Bundle Active
                      </div>
                      <h3 className="text-lg font-bold text-white">Native Offline Windows App Activated</h3>
                      <p className="text-xs text-white/70">
                        You have full access to deploy multiple terminal seats offline across your farm infrastructure.
                      </p>
                    </div>
                    <Button
                      variant="outline"
                      onClick={() => setActiveTab('licenses')}
                      className="border-emerald-500/20 text-emerald-400 hover:bg-emerald-500/10 text-xs font-bold uppercase tracking-widest px-4 py-2.5 rounded-md shrink-0 flex items-center justify-center gap-1.5"
                    >
