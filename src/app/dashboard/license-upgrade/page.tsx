...
export default function LicenseUpgradePage() {
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-11 px-3 py-14 scroll-smooth">
      {/* Header Section */}
      <div className="text-center space-y-3 max-w-2xl mx-auto">
        <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold uppercase tracking-[0.2em] mb-3">
          <Sparkles className="w-3.5 h-3.5" /> Dev Mode Enabled
        </div>
        <h1 className="text-5xl font-bold text-white tracking-normal leading-none">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-7 items-stretch pt-7">
        {tiers.map((tier) => (
          <div key={tier.name} className="relative group h-full">
            ...
            <div className="absolute -inset-1 bg-gradient-to-r opacity-20 group-hover:opacity-40 transition duration-1000 group-hover:duration-200 blur-xl rounded-lg bg-emerald-500"></div>
            
            <Card className="relative h-full bg-[#0f1115]/60 backdrop-blur-3xl border-white/5 rounded-lg overflow-hidden flex flex-col p-7 transition-all duration-500 group-hover:translate-y-[-8px]">
              <CardContent className="p-0 flex flex-col h-full space-y-7">
                {/* Tier Icon & Title */}
                <div className="space-y-3">
                  ...
                  <div>
                    <h3 className="text-2xl font-bold text-white tracking-normal">{tier.name}</h3>
                    <p className="text-sm text-white/70 font-medium">{tier.description}</p>
                  </div>
                </div>
                ...
                <div className="flex items-baseline gap-1">
                  ...
                  <span className="text-white/70 font-bold text-sm uppercase tracking-widest">{tier.period}</span>
                </div>

                {/* Features List */}
                <div className="flex-1 space-y-3">
                  <p className="text-xs font-bold text-white/20 uppercase tracking-[0.2em]">What&apos;s Included:</p>
                  <ul className="space-y-2">
                    {tier.features.map((feature) => (
                      <li key={feature} className="flex items-center gap-2 text-sm text-white/70 font-medium group-hover:text-white transition-colors">
      ...
      {/* Trust Badges */}
      <div className="pt-7 border-t border-white/5 grid grid-cols-2 md:grid-cols-4 gap-3">
        {[
          ...
          <div key={badge.label} className="flex flex-col items-center gap-2 text-center p-3">
            <badge.icon className="w-5 h-5 text-white/20" />
            <span className="text-xs font-bold text-white/70 uppercase tracking-widest leading-tight">{badge.label}</span>
          </div>
        ))}
      </div>
    </div>
