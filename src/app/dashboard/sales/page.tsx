    ...
    <div className="max-w-7xl mx-auto space-y-6 px-4 py-8">
      ...
      <div className="bg-white rounded-2xl shadow-xl shadow-gray-200/50 border border-gray-100 overflow-hidden">
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-100">
            ...
            <tbody className="divide-y divide-gray-50">
              {sales.map((sale: any) => (
                <tr key={`desk-${sale.id}`} className="transition-colors">
        ...
        <div className="md:hidden flex flex-col gap-4 p-4 bg-gray-50/30">
          {sales.map((sale: any) => (
            <div key={`mob-${sale.id}`} className="bg-white border border-gray-100 p-4 rounded-3xl shadow-lg flex flex-col gap-3 relative overflow-hidden">
              <div className="flex justify-between items-center border-b border-gray-50 pb-2">
                <span className="text-xs font-black tracking-widest text-gray-400 uppercase">{formatDate(sale.saleDate)}</span>
              ...
              <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-50">
                <h4 className="text-2xl font-black text-green-700 tracking-tighter italic">{formatCurrency(sale.totalAmount)}</h4>
                <SaleRowActions sale={sale} />
              </div>
            </div>
          ))}
